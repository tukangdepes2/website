<html>
<head>
<title>[+] Grab Domain from Zone-XSec [+]</title>
...
<body>
<center>
<font face=courier new size=5>Grab Domain from Zone-XSec<br><font size="3">Mengambil Domain dari Hasil Mirror Defacer</font><font size="3"><hr>
<form method="post">
...
if (isset($_POST['go'])) {
  ...
  echo "Defacer : ".htmlspecialchars($_POST['attacker'])."<br>";
  $cekdulu = cekinfo($attacker);
  preg_match_all("/<span>(.*?)<\/span>/im", $cekdulu, $info);
  $hasil = $info[0];
  if (preg_match("/404 Not Found/i", $cekdulu)) {
    die("Defacer Tidak Ditemukan !");
  } elseif (!preg_match("/\/mirror\/id\//i", $cekdulu)) {
    die("Defacer Tidak Ditemukan !");
  } else {
    echo $hasil[3]."<br>";
    echo $hasil[4]."<br>";
    echo $hasil[5]."<br>";
    echo $hasil[6]."<br>";
    echo "<br>[+] Loading ...<br><br>";
  }
  echo "<textarea>";
  foreach (range(1, 50) as $page) {
    $cek = attacker($attacker, $page);
    if (preg_match("/404 Not Found/i", $cek)) {
      die("Defacer tidak di temukan !");
    } elseif (!preg_match("/\/mirror\/id\//i", $cek)) {
      die("</textarea><br><br>[+] Done ....<center><hr><font size=3>Unknown45 - 2021");
    } else {
      preg_match_all("/<td>(.*?)<\/td>/im", $cek, $result);
      foreach ($result[0] as $web) {
        $web = str_replace("<td>", "", $web);
        $web = str_replace("</td>", "", $web);
        if (!preg_match("/<td|<img|<a|<i|20/i", $web)) {
          if (strlen($web) < 3) {
            continue;
          }
          $web = "//".$web;
          $parse = parse_url($web);
          if (preg_match("/No Team/i", $parse['host'])) {
            continue;
          }
          if (!empty($parse['host'])) {
...
function attacker($nick, $page) {
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "https://zone-xsec.com/archive/attacker/".$nick."/page=".$page);
...
function cekinfo($nick) {
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "https://zone-xsec.com/archive/attacker/".$nick);
  curl_setopt($ch, CURLOPT_HTTPHEADER, array("X-Requested-With: XMLHttpRequest"));
  curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.131 Safari/537.36");
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
