<html>
<head>
<title>[+] Mass WP Plugins Scanner [+]</title>
...
<body>
<center>
<font face=courier new size=5>Mass WP Plugins Scanner<br><font size="3">Check Plugins Yang di Gunakan Secara Instan</font><font size="3"><hr>
<form method="post">
...
if (isset($_POST['go'])) {
	...
	foreach ($site as $web) {
		...
		preg_match_all("/wp-content\/plugins\/(.*?)\//im", $cek, $hasil);
		$hasil = $hasil[1];
		$hasil = array_unique($hasil);
		echo "[<nemu>Wordpress</nemu>] ".htmlspecialchars($web)." (";
		foreach ($hasil as $res) {
			echo "<oren>".htmlspecialchars($res)."</oren>, ";
		}
		echo ")<br>";
	}
	echo "<br>[+] Done ...\n";
